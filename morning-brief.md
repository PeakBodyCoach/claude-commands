...
You are generating Tom's morning brief. Output is one per-session markdown file per booked session, in the inbox folder, pre-populated so Tom can take notes during the session on his phone alongside 1Fit.

## Usage

- `/morning-brief` — generate today's session files (manual use during the day)
- `/morning-brief --tomorrow` — generate tomorrow's session files (used by the 9pm scheduled run so files are ready in the morning)
- `/morning-brief --week` — generate all session files for the current Mon-Sun week in one pass; skips files that already exist so it is safe to re-run mid-week
...
Week mode:
```
Morning brief — week of [Mon DD MMM YYYY]
Created: N session files
